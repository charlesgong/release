...
run_detector() {
  ...
  TEST_CURL_URLS_FILE="${TEST_CURL_URLS_FILE:-}" \
...
test_ready_endpoint_precedes_tags_resolution() {
  local output_dir
  output_dir=$(mktemp -d)

  TEST_CURL_URLS_FILE="${output_dir}/curl-urls" \
  TEST_TAGS_FIXTURE="${FIXTURES}/tags-built.json" \
    run_detector "${output_dir}"

  assert_file_order \
    "/api/v1/releasestream/5.2.0-0.nightly/config" \
    "/api/v1/releasestreams/ready" \
    "${output_dir}/curl-urls"
  assert_file_order \
    "/api/v1/releasestreams/ready" \
    "/api/v1/releasestream/5.2.0-0.nightly/tags" \
    "${output_dir}/curl-urls"
}
...
test_empty_ready_stream_skips_tags_resolution() {
  local output_dir
  output_dir=$(mktemp -d)
  ...
  assert_file_contains "/api/v1/releasestreams/ready" "${output_dir}/curl-urls"
  assert_file_not_contains "/api/v1/releasestream/5.2.0-0.nightly/tags" "${output_dir}/curl-urls"
}
